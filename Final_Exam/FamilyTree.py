...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        if a == b:
            return (-1, 0)
        if a.is_child(b) or a.is_parent(b):
            return (-1, 0)
        else:
            while a.get_parent() != None:
                parentA = a.get_parent()
                a_ancestors.append(parentA)
                a = parentA

            while b.get_parent() != None:
                parentB = b.get_parent()
                b_ancestors.append(parentB)
                b = parentB

            removed = abs(len(b_ancestors) - len(a_ancestors))

            for anc_a in a_ancestors:
                for anc_b in b_ancestors:
                    if anc_a == anc_b:
                        break
                common = anc_a
                break
            if a_ancestors.index(common) <= b_ancestors.index(common):
                ctype = a_ancestors.index(common)
            else:
                ctype = b_ancestors.index(common)
